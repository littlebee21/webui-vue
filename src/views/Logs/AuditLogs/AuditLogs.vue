...
export default {
  ...
  computed: {
    ...
    allLogs() {
      return this.$store.getters['auditLog/allEvents'].map((event) => {
        return {
          ...event,
          actions: [
            {
  ...
  created() {
    ...
    this.timer = setInterval(() => {
      this.$store.dispatch('auditLog/getEventLogData').finally(() => {
        this.endLoader();
        this.isBusy = false;
      });
    }, 30000);
  ...
  methods: {
    changelogStatus(row) {
      this.$store
        .dispatch('auditLog/updateEventLogStatus', {
          uri: row.uri,
          status: row.status,
        })
        .then((success) => {
    ...
    deleteAllLogs() {
      this.$bvModal
        ...
        .then((deleteConfirmed) => {
          if (deleteConfirmed) {
            this.$store
              .dispatch('auditLog/deleteAllEventLogs', this.allLogs)
    ...
    deleteLogs(uris) {
      this.$store
        .dispatch('auditLog/deleteEventLogs', uris)
        .then((messages) => {
    ...
    resolveLogs() {
      this.$store
        .dispatch('auditLog/resolveEventLogs', this.selectedRows)
        .then((messages) => {
          messages.forEach(({ type, message }) => {
            if (type === 'success') {
              this.successToast(message);
    ...
    unresolveLogs() {
      this.$store
        .dispatch('auditLog/unresolveEventLogs', this.selectedRows)
        .then((messages) => {
          messages.forEach(({ type, message }) => {
            if (type === 'success') {
              this.successToast(message);
